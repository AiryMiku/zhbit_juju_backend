...
def follow(request: HttpRequest):
    ...
    if obj is not None:
        ...
        if p == 0:
            ...
            NotificationController.create_notification(1, b_id, models.User.objects.get(pk=a_id) + '关注了你')
        else:
            obj.right_to_left = True
            NotificationController.create_notification(1, a_id, models.User.objects.get(pk=b_id) + '关注了你')
        obj.save()
        return rS.success()
    else:
        if p == 0:
